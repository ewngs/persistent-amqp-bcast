...
class ExchangeWrapper extends EventEmitter {
        ...
        return self.channel
            ...
            .then(messageQueueData => {
                if (self.options.prefetch) {
                    self.channel.prefetch(self.options.prefetch);
                }
                self.messageQueueName = messageQueueData.queue;
                if (self.options.purge) {
                    logDebug('Broadcast', `Purging messages from qeueue "${self.messageQueueName}"`);
                    return self.channel.purgeQueue(self.messageQueueName);
                }
                return Promise.resolve();
            })
            .then(() => {
                logDebug('Broadcast', `Consuming messages from qeueue "${self.messageQueueName}" with options:`, self.options.consumeOptions);
    ...
    purgeQueue() {
        ...
        return this.channel.purgeQueue(this.messageQueueName);
    }
...
const exported = module.exports = function (connectionOptions, exchangeOptions) {
...
exported.shutdownClients = function () {
    Object.keys(exchanges).forEach(name => exchanges[name].shutdown());
};
